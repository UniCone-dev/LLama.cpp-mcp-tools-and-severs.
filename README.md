...
# Option A: Vulkan (Recommended for cross-platform AMD/Intel/NVIDIA GPU acceleration)

```cmake -B build -DGGML_VULKAN=ON
cmake --build build --config Release -j$(nproc)```

# Option B: CUDA (NVIDIA GPUs)

```cmake -B build -DGGML_CUDA=ON
cmake --build build --config Release -j$(nproc)```

# Option C: ROCm (AMD GPUs on Linux)

```hipcc --version
cmake -B build -DGGML_HIPBLAS=ON
cmake --build build --config Release -j$(nproc)```

# Option D: CPU Only (Universal, no GPU needed)
...
Now, simply type in your terminal:
    ...
    qwen -> Starts the LLM Inference Server on [http://127.0.0.1:8080](http://127.0.0.1:8080)

    tools-mcp -> Starts the Local MCP Server (Endpoint: [http://127.0.0.1:8000/mcp](http://127.0.0.1:8000/mcp))
...
If you prefer an AI Agent (like Claude Code, Cursor, ChatGPT Desktop, or a terminal agent) to set up everything automatically for you, simply copy-paste this prompt:
    ...
    MCP Connection Errors: Verify that the Python venv activated correctly and dependencies are installed. Ensure the MCP server URL in your client points to [http://127.0.0.1:8000/mcp](http://127.0.0.1:8000/mcp) (or the port defined in your server.py).
